...
function DataFetch($xaccounts_paymentid) {
	...
	if ($count > 0) {
		while ( $row = mysql_fetch_array ( $result ) ) {
			...
			$GLOBALS ['xaccounts_paymentid'] = $row ['accounts_payment_id'];
			$GLOBALS ['xPayment_Ledger_id'] = $row ['accounts_payment_ledger_id'];
			$GLOBALS ['xPayment_Ledger_id_For'] = $row ['accounts_payment_ledger_id_for'];
			
			$GLOBALS ['xDate'] = $row ['accounts_payment_date'];
			$GLOBALS ['xAmount'] = $row ['accounts_payment_amount'];
			$GLOBALS ['xRemarks'] = $row ['accounts_payment_remarks'];
...
function DataProcess($mode) {
	...
	$xAccountLedgerIdFor = $_POST ['f_account_ledger_id_for'];
	...
	if ($mode == 'S') {
		...
		accounts_payment_ledger_id,accounts_payment_ledger_id_for,
		accounts_payment_amount,
		accounts_payment_remarks)
		  VALUES ($xaccounts_paymentid,'$xDate',$xAccountLedgerId,$xAccountLedgerIdFor,$xAmount,'$xRemarks')";
	} elseif ($mode == 'U') {
		...
		<div class="panel panel-success">
			<div class="panel panel-info">
				<div class="panel-heading  text-center">
					<h3 class="panel-title">செலவுகள்	</h3>
				</div>
				<div class="panel-body">
					<div class="form-group">
						...
						<div class="col-xs-3">
							<label>
தேதி:</label> <input type="date" class="form-control"
								name="f_date" value="<?php echo $GLOBALS ['xDate']; ?>">
						</div>
<div class="col-xs-3">
<label>யாரிடமிருந்து :</label> <select
...
?>
	...
	<div class="col-xs-3">
<label>யாருக்காக :</label> <select
				class="form-control" name="f_account_ledger_id_for">
<?php
...
?>
						...
						<div class="col-xs-2">
							...
							<label>விலை</label> <input type="text" class="form-control"
							...
							<label>விவரங்கள் விளக்கம்</label> <input type="text" class="form-control"
								name="f_remarks" maxlength="200"
	...
	<?php include 'customtable.html';?>
				<table class="table table-hover table-bordered" border="1" id="example">
					<thead>
						<tr>
													<th width="20%">தேதி</th>
<th width="20%">யாரிடமிருந்து</th>
													<th width="20%">யாருக்காக</th>
							<th width="15%">விலை</th>
													<th width="15%">விவரங்கள்</th>
							<th  width="5%">EDIT</th>
...
while ( $row = mysql_fetch_array ( $result2 ) ) {
	...
	echo '<td>' .	$GLOBALS ['xAccountLedgerName'].  '</td>';
		fn_FindAccountLedgerDetails($row ['accounts_payment_ledger_id_for']);
	echo '<td>' .	$GLOBALS ['xAccountLedgerName'].  '</td>';
