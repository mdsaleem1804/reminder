...
$xTotalAmount=0;
    ...
    $xSellingPriceForStock = $_POST ['f_sellingprice'] / $xPackNo;
        $xOriginalPriceForStock = $_POST ['f_originalprice'] / $xPackNo;
    ...
    if ($mode == 'S') {
        ...
        if (!$retval) {
            ...
            $xMsg = "Inserted";
            fn_TempPurchaseQty(0);
            UpdateStockValues($xQtyforStock, $xItemNo, '',$xOriginalPriceForStock, $xSellingPriceForStock, $xBatchId, $xExpiredDate);
        }
    } elseif ($mode == 'U') {
        ...
        if (!$retval) {
            ...
            UpdateStockValues($xQtyforStock, $xItemNo, 'F', $xOriginalPriceForStock,$xSellingPriceForStock, $xBatchId, $xExpiredDate);
        }
    }
    $result = mysql_query("SELECT   qty,originalprice,vat FROM inv_purchaseentry where purchaseinvoiceno=$xPurchaseInvoiceNo");
...
function UpdateStockValues($xCurrentQty, $xCurrentItemNo, $mode, 
    $xCpr,$xCurrentMrp, $xCurrentBatch, $xCurrentExpiryDate) {
    ...
    if ($xItemBatchCount <= 0) {
        $xQry = "insert into inv_stockentry (stockno,itemno,stock,maxstock,mrp,batch,expdate)
				values($xStockNo,$xCurrentItemNo,
		$xCurrentQty,$xCpr,$xCurrentMrp,'$xCurrentBatch','$xCurrentExpiryDate')";
        mysql_query($xQry) or die(mysql_error());
    } else {
        if ($mode == "") {
            $xTempPurchaseQty = 0;
                    ...
                    <tbody>	
                        ...
                        while ($row = mysql_fetch_array($result2)) {
                            ...
                            <tr>
                                ...
                                <td><a
                                        href="inv_ht003purchaseentry.php<?php echo '?txno=' . $row ['txno'] . '
