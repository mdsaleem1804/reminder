...
function DataFetch($xTxNo) {
    ...
    if ($count > 0) {
        while ($row = mysql_fetch_array($result)) {
            ...
            $GLOBALS ['xMaxStock']= $row ['purchaseinvoiceno'];
...
function DataProcess($mode) {
    ...
    $xPurchaseInvoiceNo = $_POST ['f_maxstock'];
    ...
    if ($mode == 'S') {
	   ...
	   (txno,salesinvoiceno,purchaseinvoiceno,purchaseinvoicetxno,
       date,itemno,batchid, dateexpired,qty,unitrate,amount,vat,
       discountpercentage,usagestockdetails,createdason,updatedason,unitmrp)
		VALUES 
        ($xTxNo,$xSalesInvoiceNo,$xPurchaseInvoiceNo,$xPurchaseInvoiceTxNo,
		'$xCreatedDate',$xItemNo,'$xBatchId','$xDateExpired',$xQty,$xUnitRate,"
  ...
  purchaseinvoiceno=$xPurchaseInvoiceNo,"
...
function CalculateBalance()
                ...
                xmlhttp.onreadystatechange = function () {
                    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                        ...
                        document.getElementById("f_date_expired").value = a[3];
                           document.getElementById("f_maxstock").value = a[4];
  ...
  <div class="col-xs-2" style="display: none;">
                            ...
                            <input type="text" class="form-control"
                                   readonly name="f_maxstock" 
                                   id="f_maxstock"
                                   value="<?php echo $GLOBALS ['xMaxStock']; ?>">
                        </div>
                    ...
                    if ($GLOBALS ['xConfigSales_Discount'] == 'Yes') {
                        ...
                        <div class="col-xs-2">
                            <label>MRP:</label> <input type="text" 
                                                       class="form-control" 
                                                       name="f_taxincluded" id="f_taxincluded" 
